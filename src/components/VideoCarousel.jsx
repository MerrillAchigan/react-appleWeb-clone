...
const VideoCarousel = () => {
  ...
  useEffect(() => {
    let currentProgress = 0; // Changed from const to let
    const span = videoSpanRef.current;
  
    // Check if span[videoId] exists
    if (span && span[videoId]) {
      ...
      const anim = gsap.to(span[videoId], {
        ...
        onComplete: () => {
          // Update video state on completion
          if(isPlaying){
            gsap.to(videoDivRef.current[videoId],{
              width:'12px'
            })
  ...
  }, [videoId, startPlay, video]); // Added video as a dependency
  ...
  return (
    <>
            ...
            <div className='video-carousel_container'>
              ...
              <div className='absolute top-12 left-[5%] z-10'>
                {list.textLists.map((text) => (
                  <p key={text} className='md:text-2xl font-medium'>{text}</p>
                ))}
              </div>
            </div>
          </div>
      ...
      <div className='relative flex-center mt-10'>
        <div className='flex-center py-5 px-7 bg-neutral-800 backdrop-blur rounded-full'>
          {videoRef.current.map((_, i) => (
            <span key={i}
              ref={(el) => videoDivRef.current[i] = el} className='mx-2 w-3 h-3 bg-gray-600 rounded-full relative cursor-pointer'
            >
